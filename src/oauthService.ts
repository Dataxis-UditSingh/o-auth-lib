...
const providerPresets: Record<OAuthProviderName, OAuthProviderPreset> = {
  google: {
...
function normalizeProvider(config: OAuthConfig): OAuthProviderPreset {
...
async function createCodeChallenge(verifier: string, method: "S256" | "plain" = "S256") {
  if (method === "plain") {
    return verifier;
  }
...
export function parseAuthResponse(url: string): OAuthAuthResponse {
  ...
  const result: OAuthAuthResponse = {};
...
export async function createOAuthRequest(config: OAuthConfig): Promise<OAuthRequest> {
  const provider = normalizeProvider(config);
  if (!provider.authorizeUrl) {
    throw new Error("The OAuth provider authorizeUrl is required.");
  }
...
export async function openAuthPopup(
  ...
  return new Promise<OAuthAuthResponse>((resolve, reject) => {
